...
ARG VITE_WS_MEET_URL
ARG VITE_WEBSOCKET_CHAT_URL

# 전달받은 ARG로 .env.production 생성 (Vite가 자동 인식)
RUN printf "VITE_API_USER_URL=%s\nVITE_API_RECRUIT_URL=%s\nVITE_API_BOARD_URL=%s\nVITE_API_CHAT_URL=%s\nVITE_API_MEET_URL=%s\nVITE_GOOGLE_CLIENT_ID=%s\nVITE_REDIRECT_URL=%s\nVITE_WS_MEET_URL=%s\nVITE_WEBSOCKET_CHAT_URL=%s\n" \
  "$VITE_API_USER_URL" "$VITE_API_RECRUIT_URL" "$VITE_API_BOARD_URL" "$VITE_API_CHAT_URL" "$VITE_API_MEET_URL" "$VITE_GOOGLE_CLIENT_ID" "$VITE_REDIRECT_URL" "$VITE_WS_MEET_URL" "$VITE_WEBSOCKET_CHAT_URL" > .env.production
