...
const PaaSsibleUser: AxiosInstance = axios.create({
  baseURL: API_URL,
  timeout: 30000,
  withCredentials: true,
})

PaaSsibleUser.interceptors.request.use(
  (config) => {
...
PaaSsibleUser.interceptors.response.use(
  (response) => {
    return response
  },
    ...
    if (error.response?.status === 401) {
      try {
        ...
        if (error.config) {
          ...
          return PaaSsibleUser(error.config)
        }
      } catch (refreshError) {
        // refresh도 실패 → 로그아웃 처리
        ...
        clearAuthSession()
        await axios
          .post(`${API_URL}/users/auth/logout`, {}, { withCredentials: true })
          .catch(() => {})
