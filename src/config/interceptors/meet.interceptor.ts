...
const API_URL = import.meta.env.VITE_API_MEET_URL
const USER_API_URL = import.meta.env.VITE_API_USER_URL

const PaaSsibleMeet: AxiosInstance = axios.create({
  baseURL: API_URL,
  timeout: 30000,
  withCredentials: true,
})

PaaSsibleMeet.interceptors.request.use(
  (config) => {
...
PaaSsibleMeet.interceptors.response.use(
  (response) => {
    return response
  },
    ...
    if (error.response?.status === 401) {
      try {
        ...
        if (error.config) {
          ...
          return PaaSsibleMeet(error.config)
        }
      } catch (refreshError) {
        // refresh도 실패 → 로그아웃 처리
