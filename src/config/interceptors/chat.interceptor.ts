...
const PaaSsibleChat: AxiosInstance = axios.create({
  baseURL: API_URL,
  timeout: 30000,
  withCredentials: true,
})

PaaSsibleChat.interceptors.request.use(
  (config) => {
...
PaaSsibleChat.interceptors.response.use(
  (response) => {
    return response
  },
    ...
    if (error.response?.status === 401) {
      try {
        ...
        if (error.config) {
          ...
          return PaaSsibleChat(error.config)
        }
      } catch (refreshError) {
        // refresh도 실패 → 로그아웃 처리
